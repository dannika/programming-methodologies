/*
 * File: FacePamphletCanvasExtended.java
 * -----------------------------
...
public class FacePamphletCanvasExtended extends GCanvas 
	 ...
	 * the display
	 */
	public FacePamphletCanvasExtended() {
	 ...
	 * 
	 * @param FacePamphletProfileExtended profile, the profile to be displayed
	 */
	public void displayProfile(FacePamphletProfileExtended profile) {
		removeAll();
	...
	private void displayName(String profileName) {
		...
		double y = TOP_MARGIN + nameLabel.getAscent() + BANNER_HEIGHT;
		add(nameLabel, x, y);
	}
	...
	private void displayPicture(GImage profPic) {
		double x = LEFT_MARGIN;
		double y = TOP_MARGIN + nameLabelHeight + IMAGE_MARGIN + BANNER_HEIGHT;
		...
		if (profPic == null) {
			...
			double labelY = y + (IMAGE_HEIGHT / 2) + (noImage.getAscent() / 2) + BANNER_HEIGHT;
			add(rect);
	...
	private void displayStatus(String name, String status) {
		...
		double y = TOP_MARGIN + nameLabelHeight + IMAGE_MARGIN + IMAGE_HEIGHT +
				STATUS_MARGIN + userStatus.getAscent() + BANNER_HEIGHT;
		add(userStatus, x, y);
	}
	...
	/** Displays a banner consisting of a GRect and GLabel to the canvas. */
	private void displayBanner() {
		GRect banner = new GRect(0, 0, getWidth(), BANNER_HEIGHT);
		banner.setFilled(true);
		banner.setColor(Color.BLUE);
		add(banner);
		
		GLabel title = new GLabel("facepamphlet");
		title.setColor(Color.WHITE);
		title.setFont("Dialog-bold-30");
		add(title, 0, title.getHeight());
	}
